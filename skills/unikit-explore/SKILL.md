...
- `.unikit/gamedesign/GD-IDS.yaml` — the machine-readable game-design registry (only if a design workspace exists). **Schema guard:** it MUST be `version: 2` — on a pre-v2 `version: 1` registry, skip design grounding and emit a loud `WARN [design] GD-IDS.yaml is version 1 (pre-v2 layout); design grounding skipped` rather than silently misreading the old layout. Read it (when valid) for explorations touching gameplay, systems, or feature behavior, so code-side thinking is grounded in the current design (system `doc_status`, `version`, `depends_on`); the human-readable view is GAME.md's `## System Map [gen]`. Follow each entry's `source` path to `.unikit/gamedesign/systems/*.md` for detail. For explorations touching **dynamics** (player progression, pacing, onboarding, a player-facing sequence), also read `flows` for grounding — each flow's `mode`, `goals`, and `depends_on` (an empty `flows: []`, or no `flows` key yet, → skip silently, never an error); the human-readable view is GAME.md's `## Flow Map [gen]`, with detail in `.unikit/gamedesign/flows/*.md`. **One-way boundary: explore may *read* design for grounding; it never writes or edits design — that flows through the `/unikit-gd-*` skills.**
