...
| `unikit-plan`        | `.unikit/code/PLAN.md` (fast), `.unikit/code/plans/{folder}/TASKS.md` + `PLAN-BRIEF.md` (full)      | `.unikit/DESCRIPTION.md`, `.unikit/ARCHITECTURE.md`, `.unikit/ROADMAP.md`, `.unikit/code/researches/`         | `unikit-improve` may refine existing plan files                                                                                                                            |
...
- **Verify stays read-only:** `unikit-verify` reports drift and suggests owner commands; it does not update context artifacts by default. The **sole exception** is the all-AC-met `implemented` writeback to the design layer (`GD-IDS.yaml` `implemented_version` + `GD-INDEX.md` Status cell), per the `unikit-verify` row above and `gd-principles` → One-Way Boundary.
