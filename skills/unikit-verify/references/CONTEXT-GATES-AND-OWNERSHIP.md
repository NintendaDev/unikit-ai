...
**A plan manifest is edited, never rewritten.** `/unikit-implement`, `/unikit-verify` and `unikit-implement-{coordinator,worker}` touch only checkbox lines and the `## MCP Findings` table. `## Technical Context` belongs to `/unikit-plan` and `/unikit-improve`.
