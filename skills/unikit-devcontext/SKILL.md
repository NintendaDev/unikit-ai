...
- `.unikit/MCP-RECHECK-NOTES.md`, **header only** (`server:` / `audited:`), compared against that stamp. A mismatch is one `WARN [engine-mcp] notes header ≠ configured server (<notes> ≠ <configured>)` and nothing else — the entries are suspect, not void, and they still apply. Retiring them is `/unikit-mcp-audit`'s job.
