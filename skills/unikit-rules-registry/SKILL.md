...
The skill is **module-agnostic**: the knowledge base is partitioned into top-level
modules (`.unikit/memory/<module>/<tier>`), and this skill iterates over the
modules registered in `.unikit/system/modules.yml`. It never hardcodes a module
id, a tier name, or a registry layout — those come from `modules.yml`.
...
- Parse `config.rulesRegistry` as a string to guess `url` vs `local` — always read `kind` from `unikit-ai rules registry show --json`, `unikit-ai rules registry status --json`, or `unikit-ai rules status --json`.
- Hardcode the registry on-disk layout. Always derive it from each module's `enginePartitioned` flag in `modules.yml` (see "Registry layout" below).

**This skill MAY:**
- Run `unikit-ai rules *` CLI commands via `Bash`.
- Mirror the entire `.unikit/memory/<module>/<tier>/` subtree into the registry — rule files **and** reference files under nested folders like `references/`. The skill owns **writes** to the registry file tree in both create and update modes.
- Delete reference `.md` files under the registry's tier `references/` folder in update mode, and only when the reference-graph check in `update.6b` proves the file is not referenced by any rule (memory or registry). Rule `.md` files in the registry are **never** deleted by update — rule removal is a manual operation in the registry repository.
...
```
$ARGUMENTS (after stripping --module <id>)?
├── "create"  → Mode: create
├── "update"  → Mode: update
├── "sync"    → Mode: sync
...
Then run **Step 0.5** (module registry), and for `create`/`update` run **Step 0.6**
(registry write-capability gate), before jumping to the matching mode section.

## Step 0.5: Load the Module Registry

Read `.unikit/system/modules.yml` and parse every entry under `modules:` into
`{ id, tiers, enginePartitioned, skillPrefix }`. This is the authoritative list of
modules and the tiers each partitions its rules into. If the file is missing
(older install), fall back to a single module `code` with tiers `core`, `stack`
...
**Registry layout** (schema:2, derived per module from `enginePartitioned`):

- Engine-partitioned module (e.g. `code`): `<registryPath>/<module>/<engine>/<tier>`
- Non-engine module (e.g. a future `gamedesign`): `<registryPath>/<module>/<tier>`

The tier's reference subfolder is `<that tier path>/references/`. Memory sources
always live at `.unikit/memory/<module>/<tier>/`. Never hardcode these — assemble
them from the module id, its `enginePartitioned` flag, the resolved engine, and the
tier.

## Step 0.6: Registry Write-Capability Gate (create / update only)

Write modes must confirm the target registry is reachable, on the latest schema,
and writable **before** seeding or pushing. `sync` skips this gate entirely — it is
registry → memory and relies on its own `rules sync` exit codes (2/5).

Run against the target registry (the configured registry for `update`; for
`create`, run it after `create.2` against the scaffold target path):

```bash
unikit-ai rules registry status --json
```

The JSON has six facts: `{ target, kind, schema, isLatestSchema, readable, writable }`.
Exit codes: `0` (reachable, schema ≤ latest), `2` (unreachable), `5` (unsupported
schema > latest). Branch on the result:

| Condition | Verdict | Action |
|-----------|---------|--------|
| exit 2 / `schema == null` | unreachable | Report `registry unreachable` and **stop**. A write mode cannot proceed against a registry it cannot read. |
| exit 5 / `readable == false` (schema > latest) | unsupported schema | Report `registry schema is newer than this CLI — upgrade unikit-ai`, then **stop**. |
| `kind == "local"` and `isLatestSchema == false` | local, behind latest schema | Offer to migrate: `AskUserQuestion` → run `unikit-ai rules registry migrate <target>` on yes, then **re-run `rules registry status --json`** and re-evaluate. On no → **stop** (writing schema:2 layout into a schema:1 registry would corrupt it). |
| `kind == "remote"` and `isLatestSchema == false` | outdated remote | `migrate` cannot write a remote registry. Report that the user must clone the registry, run `rules registry migrate` on the clone, open a PR upstream, then point the project at a writable copy. **Stop.** |
| `kind == "remote"` and `isLatestSchema == true` | remote, read-only via CLI | A write mode needs a local registry. For `update`, this duplicates the `update.1` guard — report `update requires a local registry` and **stop**. (create scaffolds a fresh local target, so this row should not occur there.) |
| `kind == "local"` and `isLatestSchema == true` (`writable == true`) | up to date and writable | Proceed to the mode body. |

Only continue into the create-seed / update-diff steps when the gate resolves to
**writable** (`kind == "local"` and `isLatestSchema == true`).
...
| Exit | Meaning | Action |
|------|---------|--------|
| 0 | Scaffold OK | Continue to create.2b |
...
| 6 | REGISTRY_ALREADY_INITIALIZED | Ask the user: `reuse existing scaffold and continue?` → Yes proceeds to create.2b, No stops |
| 7 | PATH_OCCUPIED | Report `target path is not empty and is not a registry`, stop |
| other | Unexpected | Report stderr and stop |

### create.2b: Write-capability gate on the scaffold

`registry init` creates a fresh schema:2 scaffold, but on the **reuse** path (exit 6)
the existing directory may be a schema:1 registry. Run the **Step 0.6** gate against
`<target-path>` now. A freshly-initialized target resolves to writable immediately;
a reused schema:1 target routes through the migrate offer. Only continue to
`create.3` when the gate resolves to writable.
...
For each **module** (Step 0.5) and each **tier** of that module:

1. Use `Glob: .unikit/memory/<module>/<tier>/**/*` to enumerate the **entire subtree** — not just top-level `*.md`. This captures nested folders like `references/` that hold reference files referenced by rule headers (quickrefs, catalogs, indexes).
2. For every file discovered:
   - Compute `relPath` relative to `.unikit/memory/<module>/<tier>/`.
   - Target path = `<tier-registry-path>/<relPath>`, where `<tier-registry-path>` is the schema:2 layout from Step 0.5 (`<target-path>/<module>/<engine>/<tier>` for engine-partitioned modules, `<target-path>/<module>/<tier>` otherwise).
   - **Classify the file:**
     ...
     - **Reference file** (anything under a subdirectory, or non-`.md` extension) → copy via shell verbatim. Never touch frontmatter — reference files are plain docs and must travel with their parent rule as-is:
       1. `Bash: mkdir -p "$(dirname "<target>")" && cp "<src>" "<target>"`
3. If the tier directory yields zero files, continue silently.

Track per-module/per-tier totals: `rulesCopied` (top-level rule files) and `referencesCopied` (everything else). These feed the final table in `create.7`.

Why recursive: rule files reference reference files via `> **References**:` headers. A seed that copies only `*.md` at the top level would publish rules pointing at paths that do not exist in the registry. Mirroring the full tree keeps the references intact.
...
To make the switch complete, the skill pairs `registry set` with a follow-up `rules sync --replace --prune`. `--replace` bypasses the `source !== 'registry'` guard in `syncRulesState` so installer-sourced rules get walked; the sync loop rewrites `source`, `version`, `installed_hash`, and `origin` (via `HybridRegistry.getResolvedOrigin()`) against the new primary registry. `--prune` removes obsolete stack rules that are not in the new manifest. The HARD GUARD in `syncRulesState` still prevents discovery of previously-uninstalled rules — only already-installed entries get reclassified.
...
Table format (add a `Module` column when more than one module was seeded; omit it when only `code` ran):

```
| Module | Tier  | Rule              | Version |
|--------|-------|-------------------|---------|
| code   | core  | folders-structure | 1.0.0   |
| code   | core  | code-style        | 1.0.0   |
| code   | stack | aspid-mvvm        | 1.0.0   |
| code   | stack | node-canvas       | 1.0.0   |
```
...
### update.1b: Write-capability gate

Run the **Step 0.6** gate against `registryPath`. If the registry is local but
behind the latest schema, this routes through the `rules registry migrate` offer —
critical, because writing schema:2 layout (`<module>/<engine>/<tier>`) into a
schema:1 registry (`<engine>/<tier>`) would corrupt the layout and break
`build-manifest.js`. Only continue to `update.2` when the gate resolves to writable.
...
1. `Glob: .unikit/memory/<module>/<tier>/**/*` — walk the full subtree so reference files under `references/` (and any other nested folders) are diffed alongside their parent rules.
2. For every file `src` found in memory, compute `relPath` relative to `.unikit/memory/<module>/<tier>/` and classify it:
...
3. For every `src`:
   - target = `<tier-registry-path>/<relPath>` (schema:2 layout from Step 0.5)
...
4. Walk the registry side the same way: `Glob: <tier-registry-path>/**/*`. Any file present in the registry but not in memory becomes a **delete** candidate (handled in update.5). Tag each orphan as rule vs reference file so the prompt can display them in two groups.
...
Update is **strictly push-only** for rule files — it never removes them from the registry. For every rule `.md` file present in the registry and missing from memory (i.e. top-level `*.md` under `<tier-registry-path>` with no counterpart in `.unikit/memory/<module>/<tier>/`), append `{ module, tier, filename }` to an `orphanRuleFiles` list. That list is consumed purely by `update.8` as a WARN block — no prompt, no delete, no cascade.
...
Reconcile reference files in the registry against the real `rule → references` graph. Remove any reference `.md` under a tier's `references/` folder that no rule (in memory or in the registry) references anymore. The algorithm covers four scenarios:
...
Run the algorithm **per module + per tier** — each `<module>/<tier>` is an independent namespace and never shares reference filenames across the divide.

For each module + tier:

1. Initialise `referencedByAnyone` as an empty `Set<string>` of reference filenames.
2. Walk `<tier-registry-path>/*.md` (top-level rule files only — do not recurse into `references/` here). For every rule file found:
   - Let `ruleId = basename without .md`.
   - If `.unikit/memory/<module>/<tier>/<ruleId>.md` exists → `parseTarget = memory` (memory wins for rules present in both).
   - Else → `parseTarget = registry` (orphan parent rule — protect its own references using the on-disk registry header).
   - Read the chosen file, locate the `> **References**:` header line, and extract every reference filename with the regex `references\/([^\s,)]+\.md)/g`. This is the exact regex used by `rules-registry/scripts/build-manifest.js` — stay byte-for-byte aligned so `update.6b` and the manifest rebuild in `update.7` never disagree about what is referenced.
   - Add every extracted filename to `referencedByAnyone`.
3. Enumerate physical reference files: `Glob: <tier-registry-path>/references/*.md` → `physical` (set of filenames only, strip paths).
4. Compute `safeToDelete = physical \ referencedByAnyone` (set difference by filename).
5. For every filename in `safeToDelete`:
   - `Bash: rm "<tier-registry-path>/references/<filename>"` (single-file delete, never recursive, never glob).
   - Append `{ module, tier, filename }` to a top-level `deletedReferences` list consumed by `update.8`.
...
- **Two rules in the same tier with identical references** — the set union on `referencedByAnyone` handles this: a shared file is recorded once and protected by both rules.
...
- The next `unikit-ai rules sync` (Safe mode) will mark disk files as `locally-modified` and **skip** the version update, so the drift never heals on its own.

Force-install every touched rule in a single variadic CLI call. Reference files are **not** listed — `installOneRule` re-fetches a parent rule's declared references automatically during force install.
...
`installOneRule` with `force=true` covers all three state branches deterministically:
...
A free bonus: `rulesInstallCommand` always regenerates `RULES_INDEX.md` at the end of its invocation, so the skill does not need a separate index refresh after this step.
...
Table format (add a `Module` column when more than one module was pushed; omit it when only `code` ran):

```
| Action | Module | Tier  | Rule        | Version       |
|--------|--------|-------|-------------|---------------|
| add    | code   | stack | aspid-mvvm  | 1.0.0         |
| update | code   | stack | node-canvas | 1.2.0 → 1.3.0 |
| skip   | code   | core  | code-style  | 1.1.0         |
```
...
- If `Z > 0`, print a separate deletion table directly under the tally line:
  ...
  ```
  | Action | Module | Tier  | File                         |
  |--------|--------|-------|------------------------------|
  | delete | code   | stack | aspid-mvvm-binders-full.md   |
  | delete | code   | core  | old-reference.md             |
  ```

  Rows come from `deletedReferences`, sorted by `module`, then `tier`, then `filename`. Do not inline the list on one line — the table is both the primary signal and the full record.
...
⚠️ WARN: registry contains orphan rule files not mirrored in .unikit/memory/:
  - code/stack/legacy-rule.md
  - code/core/retired-guidelines.md
...
This mode does **not** run the Step 0.6 write-capability gate — it does not write the
registry. `rules sync` carries its own reachability/validation exit codes (2/5),
which `sync.2` handles. `rules sync` reconciles the whole project and does **not**
accept `--module`; a resolved `--module <id>` does not narrow the CLI call in this
version (it only documents intent for the report).
...
`rules sync` does not accept `--module` — do not append it. The command reconciles
every module's rules in one pass.
...
Table format (add a `Module` column when more than one module synced; omit it when only `code` ran):
...
- `<registry-path>/<module>/<engine>/<tier>/**` (engine-partitioned modules) or `<registry-path>/<module>/<tier>/**` (non-engine modules) — rule files and any nested reference files (e.g. `references/*.md`)
...
- A tier's `references/*.md` — reference files. Only when the reference-graph check in `update.6b` proves the file is not referenced by any rule in memory or in the registry (shared-reference protection).
- Rule `.md` files in the registry are **never** deleted by update — rule removal is a manual operation in the registry repository.
...
- `.unikit/system/modules.yml` — module registry snapshot, regenerated by the build pipeline
