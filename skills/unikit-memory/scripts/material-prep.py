...
Single self-contained file (no sibling modules, no package): all tunables and
stable literals live in the CONSTANTS section near the top, followed by the
ExtractedDocument record, the safety/text/XML/zip helpers, the chunker
(structure-blind greedy packing + heading-aware section packing), the FB2/EPUB
book extractors, the source acquisition + extraction routing, the output stage
(chunk files + manifest + ## TOC index + marker-gated cleanup), and finally the
argparse entrypoint.

Extraction strategy notes:
  - Book formats use the standard library only (no new hard dependencies):
    FB2 = xml.etree (+ zipfile for the .fb2.zip container); EPUB = zipfile
    (+ the html flattener, used carefully to preserve <h1..h3> as headings).
  - FB2/EPUB/markdown GUARANTEE markdown headings (#/##/###) in the returned
    text so the heading-aware chunker can build a real ## TOC.
"""
...
# Tool marker identity (written into every output dir; gates reuse + cleanup).
...
MARKER_OWNERSHIP_NOTE = (
    "This directory is generated working output and may be removed by material-prep.py --cleanup."
)

# Output layout filenames.
MANIFEST_FILENAME = "manifest.json"
CHUNKS_DIRNAME = "chunks"
INDEX_FILENAME = "source-index.md"

# Temp-dir prefixes (output dir + per-run URL download scratch).
TEMP_DIR_PREFIX = "unikit-memory-"
DOWNLOAD_DIR_PREFIX = "unikit-memory-download-"

# Chunking / hashing tunables.
DEFAULT_CHUNK_CHARS = 18000  # approximate max characters per chunk
CHUNK_INDEX_WIDTH = 4        # zero-padding width for the global chunk index in filenames
DOC_HASH_LENGTH = 12         # truncated sha256 length recorded per document
MAX_HEADING_LEVEL = 3        # FB2 section nesting is capped at this heading depth
SLUG_MAX_LENGTH = 80         # max length of a generated slug

# Network.
URL_TIMEOUT_SECONDS = 60
HTTP_USER_AGENT = "unikit-memory/1.0"
DEFAULT_DOWNLOAD_NAME = "downloaded-source"
DEFAULT_DOWNLOAD_EXT = ".html"

# Text decoding fallback order for raw bytes of unknown encoding.
DECODE_ENCODINGS = ("utf-8", "utf-8-sig", "cp1251", "latin-1")
...
# Module-level compiled regexes (heading detection for the chunker + EPUB extractor).
...
# ─────────────────────────────────────────────────────────────────────────────
# Safety: path / text / XML / archive helpers
# ─────────────────────────────────────────────────────────────────────────────
...
def slugify(value: str, fallback: str = "source") -> str:
    ...
    value = value.strip("-")
    return value[:SLUG_MAX_LENGTH] or fallback
...
# ─────────────────────────────────────────────────────────────────────────────
# Book extractors (FB2 / EPUB) — standard library only, headings guaranteed
# ─────────────────────────────────────────────────────────────────────────────
...
def _fb2_render(node, depth: int, lines: list[str]) -> None:
    ...
    for child in node:
        ...
        if name == "section":
            ...
            if title:
                lines.append("")
                lines.append(f"{'#' * min(depth, MAX_HEADING_LEVEL)} {title}")
                lines.append("")
...
# ─────────────────────────────────────────────────────────────────────────────
# Source acquisition + extraction: PDF/HTML/text/book routing, URL download, walk
# ─────────────────────────────────────────────────────────────────────────────
...
def download_url(url: str, work_dir: Path) -> Path:
    ...
    basename = Path(urllib.parse.unquote(parsed.path)).name or DEFAULT_DOWNLOAD_NAME
    if "." not in basename:
        basename += DEFAULT_DOWNLOAD_EXT
...
# ─────────────────────────────────────────────────────────────────────────────
# Output stage: chunk files, manifest.json, source-index.md (## TOC), marker
# ─────────────────────────────────────────────────────────────────────────────
...
def write_chunks(docs: list[ExtractedDocument], out_dir: Path, chunk_chars: int) -> dict:
    ...
    chunk_index = 1
    for doc in docs:
        doc_hash = hashlib.sha256(doc.text.encode("utf-8", errors="ignore")).hexdigest()[:DOC_HASH_LENGTH]
        ...
        for local_index, chunk in enumerate(doc_chunks, start=1):
            ...
            filename = f"{chunk_index:0{CHUNK_INDEX_WIDTH}d}-{slug}.md"
            ...
            header = [
                f"# Chunk {chunk_index:0{CHUNK_INDEX_WIDTH}d}",
                "",
                f"Source: {doc.source}",
        ...
        json.dumps(
            {
                ...
                "ownership": MARKER_OWNERSHIP_NOTE,
            },
            indent=2,
        )
        + "\n",
...
# ─────────────────────────────────────────────────────────────────────────────
# Entrypoint: argument parsing + run-once orchestration + top-level error guard
# ─────────────────────────────────────────────────────────────────────────────
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--chunk-chars", type=int, default=DEFAULT_CHUNK_CHARS, help="Approximate max characters per chunk.")
...
def main() -> int:
    ...
    out_dir = Path(args.out).expanduser().resolve() if args.out else Path(tempfile.mkdtemp(prefix=TEMP_DIR_PREFIX))
    prepare_output_dir(out_dir)

    with tempfile.TemporaryDirectory(prefix=DOWNLOAD_DIR_PREFIX) as download_dir:
    ...
    (out_dir / MANIFEST_FILENAME).write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    write_index(out_dir, manifest)

    print(f"Output: {out_dir}")
    print(f"Documents: {len(manifest['documents'])}")
    print(f"Chunks: {len(manifest['chunks'])}")
    print(f"Index: {out_dir / INDEX_FILENAME}")
    return 0
