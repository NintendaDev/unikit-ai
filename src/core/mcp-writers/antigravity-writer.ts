...
function toAntigravityServerConfig(rawConfig: Record<string, unknown>): Record<string, unknown> {
...
export class AntigravityMcpWriter implements McpWriter {
  ...
  upsert(settings: Record<string, unknown>, key: string, config: Record<string, unknown>): void {
    ensureNestedRecord(settings, 'mcpServers')[key] = toAntigravityServerConfig(config);
  }
