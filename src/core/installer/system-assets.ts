...
import {
  ...
  GAMEDESIGN_GENRES_DIR_NAME, MODULES_YML_FILE, ENGINE_MCP_DIR_NAME, type EngineMcpShard,
...
// --- Engine-MCP shard installation ---

/**
 * Common preamble prepended to every shard file. Holds the two rules that are
 * true for every MCP server (§C1 of the research) so no individual shard has to
 * repeat them, plus the "do not hand-edit" notice — the folder is flat-rewritten
 * on every init/update.
 */
function renderEngineMcpShardHeader(shard: EngineMcpShard): string {
  return [
    `# Engine MCP — ${shard}`,
    '',
    'Generated by `unikit-ai` from the MCP servers selected for this project.',
    'Editing it by hand is pointless — every `init` / `update` rewrites this folder.',
    '',
    '## Rules that hold for every MCP server',
    '',
    '- Verify by READ-BACK, not by the response code. `success: true` is not evidence:',
    '  silent no-ops are confirmed in 2 of the 4 supported engine MCP servers.',
    '- Do not invent tool names. Each list below was checked against the version stamped',
    '  in the server\'s `verified` field; if a tool is missing, say so — do not substitute',
...
/**
 * Write the collected engine-MCP shards into `.unikit/system/engine-mcp/`.
 *
 * Symmetric with {@link installGenreProfiles}: the caller (`init` / `update`)
 * collects state — here, `collectMcpShards(discoveredServers, selectedFileIds)`
 * — and this installer only writes it. Keeping the collection out means
 * `system-assets.ts` never has to know about `mcp-shards.ts`.
 *
 * NOT hash-tracked; every init/update rewrites the folder. Orphan-delete is
 * wider than the gamedesign shards': **every** `*.md` absent from the current
 * set is removed, because that is the only thing standing between a project
 * that switched engine (or swapped its MCP server) and a stale Unity profile —
 * system assets have no migration chain. An empty set cleans the folder and
 * does not create it: an engine with no shard-contributing MCP is a normal
 * state, and the skills skip a missing shard silently.
 */
export async function installEngineMcpShards(
  projectDir: string,
  shards: Map<EngineMcpShard, string>,
): Promise<void> {
  const destDir = systemEngineMcpDir(projectDir);
  const wanted = new Set<string>();

  for (const [shard, body] of shards) {
    const fileName = `${shard}.md`;
    await writeTextFile(path.join(destDir, fileName), renderEngineMcpShardHeader(shard) + body.trim() + '\n');
    wanted.add(fileName);
  }

  // Orphan-delete every *.md the current selection did not contribute.
  for (const name of (await listFiles(destDir)).filter(n => n.endsWith('.md'))) {
    if (!wanted.has(name)) {
      await removeFile(path.join(destDir, name));
      logInfo('installEngineMcpShards', `removed orphan shard ${name}`);
    }
  }

  if (wanted.size === 0) {
    logInfo('installEngineMcpShards', 'no MCP shards contributed, skipping');
    return;
  }

  logInfo('installEngineMcpShards', `installed ${wanted.size} shard(s) into .unikit/system/${ENGINE_MCP_DIR_NAME}/`);
}
