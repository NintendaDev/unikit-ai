...
2. **Installing skills and subagents** - 34 skills (23 code-pipeline + 11 game-design) + 8 background agents (sidecars, coordinators, workers) tailored to the selected engine, grouped by category so you only install what you need
...
| **Code** | `unikit-architecture`, `unikit-archive`, `unikit-commit`, `unikit-devcontext`, `unikit-evolve`, `unikit-explore`, `unikit-fix`, `unikit-implement`, `unikit-improve`, `unikit-plan`, `unikit-review`, `unikit-roadmap`, `unikit-verify` |
...
23 skills form the code pipeline (Core + Memory and rules + Code + Tools); the 11 Game Design skills are a separate, optional module - see [Skills Reference](skills.md) for the full per-skill breakdown.
...
│   ├── skills/                   # 34 skills (or fewer, per your Step 3 selection)
...
│   ├── rules/                    # Project rule topics, listed in RULES.md - managed by /unikit-rules
