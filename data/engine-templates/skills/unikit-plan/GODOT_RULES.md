...
Engine planning vocabulary for Godot 4 projects. This file tells the `unikit-plan` skill **how to write a plan** for this engine — how the neutral `Editor:` grammar resolves into Godot concepts, which placeholders the plan templates expand to, and when a change belongs in `Editor:` rather than `Files:`.

## §1 Kind → concept

The `Editor:` line is `Editor: [kind] <container> → <target> : <action>`. For Godot, `<container>` and `<target>` resolve as follows:

| Kind | Container | Target | Typical assets | Affordance the editor must offer |
|------|-----------|--------|----------------|----------------------------------|
| `scene` | a `.tscn` scene, including an inherited scene | a node by path from the scene root (`/Main/Child`), or the script attached to it | `.tscn` | rebuild the node tree · attach / detach a script · write an exported property · wire a node path or a resource reference |
...
| `asset` | a `.tres` / `.res` Resource, including a custom Resource type | the resource itself, or one of its exported fields | `.tres`, `.res` | create a resource of a named type · write an exported field · read it back |
...
| Placeholder | Godot value |
|-------------|-------------|
| code fence (`<lang>`) | `gdscript` |
| source extension (`<ext>`) | `gd` |
...
Write an `Editor:` line when the change touches the editor's **serialized state**. Eight concrete signals for Godot — any one of them is enough:
...
**Negative rule.** Writing or changing a `.gd` source file is always `Files:`, never `Editor:` — **whatever route ends up writing the file**. Choosing the write route belongs to the implementer, not to the planner, and it does not turn a source file into an editor target. The only criterion is the one in the neutral principles: the boundary is serialized editor state. Editing a plain text or config file likewise stays in `Files:` — a `.gd`, a `.json`, a `.md`, a `.cfg` is not editor state even though it lives under `res://`. Writing a script that will later be attached to a node is `Files:` (the attaching is the `Editor:` half).
