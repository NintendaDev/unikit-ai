...
- `.unikit/gamedesign/GD-IDS.yaml` — the machine-readable roster: `systems`,
  `flows`, and `events`. It is the source of truth for ids, `status`,
  `doc_status`, `version`, `implemented_version`, `depends_on`, `goals`/`targets`,
  and `source` paths. Read this, not the maps that render from it.
...
- `GAME.md` `## System Map [gen]` / `## Flow Map [gen]` / `## Funnel [gen]` —
  **read-only renders**. Use them only to orient; never treat a render as the
  source of a fact that already lives in `GD-IDS.yaml`.
...
A request can target either authoring axis (a **system** or a **flow**). **Intent
decides the door** — resolve the axis the request names; do **not** default to
systems:
...
- **Ambiguous** (reads as either axis, or names both) → `AskUserQuestion` listing
  the candidates; **never guess** the axis.
...
`depends_on`/`goals[].targets`); they must not re-author the ladder.
...
`goals[].targets`); a resolved flow depends on systems. Walk that edge per the
consumer's own mechanics — this contract fixes only the door-choice rule above.
The phrasing mirrors the `/unikit-gd-explore` routing stance (intent-inferred,
never a flag; ambiguity → ask).
