...
disagreement; a stale render is a *freshness* issue verify PRINTS (the owner re-renders on its next write), not a
conflict to resolve.
...
| **Map freshness (3-surface)** | `GAME.md` `## System Map [gen]` | the render carries a **phantom `SYS-ghost` row** with no `GD-IDS.yaml` entry — a stale render. Verify is read-only: it **prints** the stale row (the owner re-renders on its next write) — it does **not** re-render here, and does **not** file a resolvable conflict |
...
| **Status coherence** (2-place) | `SYS-boost` | header `> Status: skeleton`, but `GD-IDS` `doc_status: detailed` — the two places that must agree disagree |
...
| **Placeholder leak** | `systems/boost.md` §E | a `detailed` document (per `GD-IDS` — the machine truth) still carries a `[To be designed]` skeleton placeholder |
...
| **Flow status coherence** (2-place) | `FLOW-first-run` | header `> Status: skeleton`, but `GD-IDS` `flows[].doc_status: detailed` |
...
| **Flow map freshness (3-surface)** | `GAME.md` `## Flow Map [gen]` | a **phantom `FLOW-ghost` row** with no `GD-IDS` entry — verify is read-only: it **prints** the stale row (the owner re-renders on its next write, recomputing the derived `Realized` column from the depended-on systems' `implemented_version`), not a resolvable conflict |
...
| **Content map freshness (3-surface)** | `GAME.md` `## Content Map [gen]` | a **phantom `CT-ghost` row** with no `GD-IDS` entry — verify is read-only: it **prints** the stale row (the owner re-renders from `content_types` on its next write), not a resolvable conflict |
...
| **Cross-axis SYS→CT staleness** | `CT-spawn` ↔ `SYS-combat` | `CT-spawn` `belongs_to SYS-combat`; with `SYS-combat` as the **changed scope**, verify's impact pass must **print** `CT-spawn` as affected (informational — verify is read-only, it never marks a status; `CT-spawn` stays `detailed`) and recommend a re-author pass to `unikit-gd-content` |

The `## Content Map [gen]` render is otherwise faithful to `GD-IDS` (CT-spawn shows
`Ver 2`, CT-card `detailed`), so the **Content status/version** conflict above is a
...
|-----------|-------|------------------------------------|
| **Display precedence** | `SYS-hud` | `GD-IDS` marks it `status: deprecated`, so the `## System Map [gen]` Status shows `deprecated` over the underlying `doc_status: detailed`; the header (`detailed`) and `GD-IDS` `doc_status` (`detailed`) agree. The `deprecated` display overlay is excluded from the two-place Status comparison. |
...
`/unikit-gd-verify apply-phase3`. That closing pass must **print + report only** (verify is read-only) — it
